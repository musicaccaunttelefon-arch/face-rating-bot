...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохраняем id фото чтобы удалить при нажатии кнопки
    if photo_msg_id:
        user_data_store[user_id] = user_data_store.get(user_id, {})
        user_data_store[user_id]["welcome_photo_id"] = photo_msg_id
        user_data_store[user_id]["chat_id"] = update.message.chat_id
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Удаляем приветственное фото если есть
    ...
    if stored.get("welcome_photo_id"):
        try:
            ...
            pass
        user_data_store[user_id].pop("welcome_photo_id", None)
        user_data_store[user_id].pop("chat_id", None)
