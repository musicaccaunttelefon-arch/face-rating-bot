...
ADMIN_USER_ID  = None  # Заполнится автоматически при первом /start от админа
...
def is_admin(username, user_id=None):
    if user_id and ADMIN_USER_ID and user_id == ADMIN_USER_ID:
        return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (username or "").lower() == ADMIN_USERNAME.lower() and not ADMIN_USER_ID:
        ADMIN_USER_ID = user_id
        logger.info(f"Admin user_id set: {user_id}")
