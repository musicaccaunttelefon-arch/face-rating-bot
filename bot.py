...
# user_id -> {file_id, mode, gender, opponent, score, category, emoji}
user_data_store = {}

# Очереди по полу: "male" -> [user_id, ...], "female" -> [user_id, ...]
matchmaking_queues = {"male": [], "female": []}

# Матчи: user_id -> {opponent_id, my_score, opp_score, my_cat, opp_cat, ...}
match_store = {}
...
def get_player(user_id):
    conn = sqlite3.connect("ratings.db")
    c = conn.cursor()
    c.execute("SELECT username,score,category,wins,losses,matches,gender FROM players WHERE user_id=?", (user_id,))
    row = c.fetchone()
    conn.close()
    return row
...
def analyze_face(image_bytes: bytes, gender: str) -> dict:
    try:
        ...
        total   = max(0, min(100, total + random.uniform(-4,4)))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "😴 *Если бот не отвечает* — напиши /start и подожди 30 секунд.\n\n"
        "Выбери действие:",
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "match":
        # В матче пол уже известен из mm_gender
        gender = mm_gender
        keyboard = [[
        ...
        await update.message.reply_text(
            f"Фото получено! Нажми чтобы подтвердить:",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "profile":
        ...
        if not row:
            ...
            return
        uname, score, cat, wins, losses, matches, gender = row
...
async def start_matchmaking(query, user_id, user, context, mm_gender):
    ...
    if opponent_id:
        ...
        # Сохраняем данные матча для обоих
        match_store[user_id]     = {"opponent_id": opponent_id, "my_score": None, "opp_score": None,
                                     ...
                                     "opp_name": user_data_store.get(opponent_id, {}).get("opp_name", "Аноним")}
        ...
        try:
            await context.bot.send_message(
                ...
                text=f"✅ Соперник найден!\n\n"
                     f"⚔️ Против: *{user.first_name or 'Аноним'}*\n\n"
                     f"📸 Пришли своё фото для оценки!",
                parse_mode="Markdown"
            )
        except Exception as e:
...
async def process_match_result(query, user_id, score, category, emoji, context):
    ...
    # Сохраняем свой результат
    match["my_score"] = score
    match["my_cat"]   = category
    match["my_emoji"] = emoji

    # Записываем результат и в данные соперника
    ...
    if opp_score is None:
        # Соперник ещё не прислал фото
        await query.edit_message_text("✅ Фото принято! Ждём фото соперника...")
        try:
            await context.bot.send_message(
                chat_id=opponent_id,
                text="⏳ Соперник уже прислал фото. Пришли своё!"
            )
        except:
            pass
        return
    ...
    try:
        await context.bot.send_message(
            chat_id=opponent_id, text=opp_text,
    ...
    # Чистим матч
    match_store.pop(user_id, None)
    match_store.pop(opponent_id, None)
    user_data_store.pop(user_id, None)
