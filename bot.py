...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("premium_analysis"):
        ...
        eye_sp = result.get("_eye_spacing", "")
        fwhr_v = result.get("_fwhr", "")
        fwhr_line = f"• fWHR: `{fwhr_v}`\n" if fwhr_v else ""
        eye_line  = f"• Межглазное расстояние: `{eye_sp}`\n" if eye_sp else ""
        text = (
            f"🔬 *Расширенный анализ*\n\n"
            f"🎯 Тир: *{result.get('tier','?')}* ({result.get('score','?')}/10)\n\n"
            f"📐 *Пропорции лица:*\n"
            f"• Физиогн. индекс: `{fi}`\n"
            f"{fwhr_line}"
            f"• Трети: `{upper_t}% / {mid_t}% / {lower_t}%`\n\n"
            f"👁 *Глаза:*\n"
            f"• Canthal Tilt: `{canthal_a}°` — {canthal_c}\n"
            f"{eye_line}"
            f"\n🦷 *Челюсть:*\n"
            f"• Гониальный угол: `{gonial_a}°` {gonial_c}\n\n"
            f"📊 *Симметрия:* `{asym}%` — {asym_cls}\n"
            f"🧲 *Скулы:* {malar}\n"
            f"👃 *Носогубный угол:* `{nl_angle}°` — {nl_status}\n"
            f"🪞 *Текстура кожи:* {texture}\n\n"
