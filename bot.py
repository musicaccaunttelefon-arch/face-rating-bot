...
def analyze_face(image_bytes: bytes, gender: str) -> dict:
    try:
        ...
        phi_diff = abs(fh / max(fw, 1) - 1.618) / 1.618
        ...
        face_area = fw * fh
        scores["face_clarity"] = min(100, face_area / max(w*h, 1) * 400)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "matchmaking":
        row = get_player(user_id)
        if not row:
            await query.edit_message_text("⚔️ Сначала пройди оценку внешности!", reply_markup=main_menu_keyboard())
            return
...
async def start_matchmaking(query, user_id, user, context):
    ...
    if opponent_id:
        ...
        await query.edit_message_text(
            f"✅ Соперник найден!\n\n⚔️ Против: *{opponent_info.get('name','Аноним')}*\n\n📸 Пришли своё фото!",
            parse_mode="Markdown"
        )
        try:
...
# ──────────────────────────────────────────────
# ВЕБ-СЕРВЕР (чтобы Render не падал по таймауту)
# ──────────────────────────────────────────────

RENDER_URL = os.environ.get("RENDER_URL", "")  # например https://face-rating-bot.onrender.com
...
async def self_ping():
    ...
    if not RENDER_URL:
        logger.info("RENDER_URL не задан, самопинг отключён.")
        return
    import aiohttp
    await asyncio.sleep(30)  # подождём пока сервер стартует
    while True:
        try:
