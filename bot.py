...
def get_all_profiles(offset=0, premium_only=False):
    ...
    if premium_only:
        c.execute("""SELECT user_id,score,category,gender,wins,losses,matches,profile_file_id,is_premium
                     FROM players WHERE is_premium=TRUE ORDER BY score DESC LIMIT 1 OFFSET $1""", [offset])
        c2 = conn.cursor()
        c2.execute("SELECT COUNT(*) FROM players WHERE is_premium=TRUE")
    else:
        c.execute("""SELECT user_id,score,category,gender,wins,losses,matches,profile_file_id,is_premium
                     FROM players ORDER BY score DESC LIMIT 1 OFFSET $1""", [offset])
        c2 = conn.cursor()
...
def check_edit_level(img_gray):
    ...
    # Тёмные фото имеют низкую дисперсию не из-за обработки — корректируем
    brightness_factor = max(0.3, mean_bright / 128)
    adjusted = lap_var * brightness_factor
    return round(max(0, min(100, 100 - min(adjusted / 3, 100))), 1)

def check_screen_photo(img_bgr):
    import cv2
    # Более строгий порог — только явные скрины
    ...
    return (outer.mean() / max(center.mean(), 1)) > 0.35  # было 0.15, теперь строже
...
def analyze_face(image_bytes, gender):
    try:
        ...
        # Пробуем найти лицо, при неудаче — повышаем контраст и пробуем снова
        faces = face_cascade.detectMultiScale(img_gray, 1.1, 5, minSize=(60, 60))
        if len(faces) == 0:
            img_eq = cv2.equalizeHist(img_gray)
            faces  = face_cascade.detectMultiScale(img_eq, 1.1, 4, minSize=(60, 60))
        if len(faces) == 0:
            return {"error": "Лицо не обнаружено. Пришли чёткое фото анфас при нормальном освещении."}

        fx, fy, fw, fh = max(faces, key=lambda f: f[2] * f[3])

        # Находим глаза ДО обрезки — для выравнивания наклона
        face_top_raw = img_gray[fy: fy + fh // 2, fx: fx + fw]
        eyes_raw = eye_cascade.detectMultiScale(face_top_raw, 1.05, 3)
        rotation_angle = 0.0
        eye_data = None
        if len(eyes_raw) >= 2:
            eyes_raw = sorted(eyes_raw, key=lambda e: e[0])
            ex1, ey1, ew1, eh1 = eyes_raw[0]; ex2, ey2, ew2, eh2 = eyes_raw[1]
            cx1 = fx + ex1 + ew1 // 2; cy1 = fy + ey1 + eh1 // 2
            cx2 = fx + ex2 + ew2 // 2; cy2 = fy + ey2 + eh2 // 2
            rotation_angle = math.degrees(math.atan2(cy2 - cy1, cx2 - cx1))
            eye_data = (cx1, cy1, cx2, cy2, ew1, ew2)

        # Выравниваем изображение по наклону глаз если > 3 градусов
        if abs(rotation_angle) > 3:
            center  = (w // 2, h // 2)
            mat     = cv2.getRotationMatrix2D(center, rotation_angle, 1.0)
            img_gray = cv2.warpAffine(img_gray, mat, (w, h))
            img_bgr  = cv2.warpAffine(img_bgr,  mat, (w, h))
            # Переопределяем лицо после поворота
            faces2 = face_cascade.detectMultiScale(img_gray, 1.1, 5, minSize=(60, 60))
            if len(faces2) > 0:
                fx, fy, fw, fh = max(faces2, key=lambda f: f[2] * f[3])
            # Переопределяем глаза
            face_top_raw2 = img_gray[fy: fy + fh // 2, fx: fx + fw]
            eyes_raw2 = eye_cascade.detectMultiScale(face_top_raw2, 1.05, 3)
            if len(eyes_raw2) >= 2:
                eyes_raw2 = sorted(eyes_raw2, key=lambda e: e[0])
                ex1, ey1, ew1, eh1 = eyes_raw2[0]; ex2, ey2, ew2, eh2 = eyes_raw2[1]
                cx1 = fx + ex1 + ew1 // 2; cy1 = fy + ey1 + eh1 // 2
                cx2 = fx + ex2 + ew2 // 2; cy2 = fy + ey2 + eh2 // 2
                eye_data = (cx1, cy1, cx2, cy2, ew1, ew2)

        # Обрезаем bbox — срезаем волосы/поля
        pad_x     = int(fw * 0.10)
        pad_y_top = int(fh * 0.10)
        x1 = fx + pad_x; x2 = fx + fw - pad_x
        y1 = fy + pad_y_top; y2 = fy + fh
        face_img = img_gray[y1:y2, x1:x2]
        fc_w = x2 - x1; fc_h = y2 - y1
        if fc_w < 10 or fc_h < 10:
            return {"error": "Лицо слишком маленькое на фото."}

        scores = {}

        # 1. Яркость лица — штраф за слишком тёмное фото
        mean_bright = float(face_img.mean())
        brightness_ok = mean_bright >= 60  # 0-255
        brightness_penalty = max(0, (60 - mean_bright) / 60) if not brightness_ok else 0

        # 2. Симметрия — с нормализацией яркости
        face_norm = cv2.equalizeHist(face_img)  # нормализуем перед сравнением
        mid        = fc_w // 2
        left_half  = face_norm[:, :mid]
        right_half = cv2.flip(face_norm[:, mid:], 1)
        min_w      = min(left_half.shape[1], right_half.shape[1])
        diff       = cv2.absdiff(left_half[:, :min_w].astype(float), right_half[:, :min_w].astype(float))
        raw_sym    = max(0, 100 - diff.mean() / 128 * 160)
        # Штраф за тёмное фото — симметрия ненадёжна
        scores["symmetry"] = min(95, raw_sym * (1 - brightness_penalty * 0.4))

        # 3. Пропорции
        fi = fc_h / max(fc_w, 1)
        ideal_min = 1.30 if gender == "male" else 1.35
        ideal_max = 1.55 if gender == "male" else 1.65
        if ideal_min <= fi <= ideal_max:
            scores["golden_ratio"] = 88
        elif fi < ideal_min:
            scores["golden_ratio"] = max(35, 88 - (ideal_min - fi) * 120)
        else:
            scores["golden_ratio"] = max(35, 88 - (fi - ideal_max) * 120)

        # 4. Глаза
        if eye_data:
            cx1, cy1, cx2, cy2, ew1, ew2 = eye_data
            inter_eye = abs(cx2 - cx1)
            avg_eye_w = (ew1 + ew2) / 2
            eye_ratio = inter_eye / max(avg_eye_w, 1)
            scores["eye_spacing"] = max(0, 100 - abs(eye_ratio - 2.5) / 2.5 * 75)
            scores["eye_level"]   = max(0, 100 - abs(cy1 - cy2) / max(fc_h, 1) * 300)
        else:
            scores["eye_spacing"] = 50
            scores["eye_level"]   = 50

        # 5. Чёткость
        scores["face_clarity"] = min(100, fc_w * fc_h / max(w * h, 1) * 500)

        # Веса
        weights = {"symmetry": 0.30, "golden_ratio": 0.20, "eye_spacing": 0.20, "eye_level": 0.10, "face_clarity": 0.20}
        total = max(0, min(100, sum(scores[k] * weights[k] for k in weights)))

        # Штраф за тёмное/плохое фото — результат менее точен
        total = total * (1 - brightness_penalty * 0.15)

        # Реальные люди редко ниже 35
        total = max(35, total)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mode      = stored.get("mode", "rate")
    mm_gender = stored.get("mm_gender")
    step      = stored.get("step")

    # Dating FSM photo — обрабатываем ДО перезаписи store
    if mode == "dating_fsm" and step == "photo":
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("lb_male","lb_female","lb_premium"):
        ...
        for i, (uname, score, cat, wins, losses, is_prem) in enumerate(rows):
            ...
            display  = f"@{uname}" if uname and not uname.isdigit() else (uname or "Аноним")
            text  += f"{medal} {display}{p_icon} — *{cat_name}* ({score:.0f} б) | {wins}W/{losses}L\n"
    ...
    if data == "profile":
        ...
        cat = cat[0] if isinstance(cat, tuple) else (cat or "")
        ...
        if history:
            ...
            for h_score, h_cat, h_date in history:
                h_cat = h_cat[0] if isinstance(h_cat, tuple) else (h_cat or "")
                hist_text += f"• {str(h_date)[:10]} — *{h_cat.upper()}* ({h_score:.0f} б)\n"
        text = (
    ...
    if data == "back_menu":
        try:
            ...
            try:
                await query.edit_message_caption(caption="Выбери действие:", reply_markup=menu())
            except Exception:
                await context.bot.send_message(chat_id=query.message.chat_id, text="Выбери действие:", reply_markup=menu())
        return
    ...
    if data.startswith("browse_") or data.startswith("premium_browse_"):
        ...
        if profile_file_id:
            try:
                # Пробуем отредактировать медиа если предыдущее сообщение тоже фото
                from telegram import InputMediaPhoto
                await query.edit_message_media(
                    media=InputMediaPhoto(media=profile_file_id, caption=text, parse_mode="Markdown"),
                ...
                await context.bot.send_photo(
                    chat_id=query.message.chat_id, photo=profile_file_id,
    ...
    if data == "admin":
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🎯 Выдать рейтинг",    callback_data="give_score")],
    ...
    if data == "give_score":
        if not is_admin(username): return
        user_data_store[user_id] = {"mode": "awaiting_give_score"}
        await query.edit_message_text("Введи ID пользователя, пол (male/female) и рейтинг через пробел:\nПример: `1495553178 male 85.5`", parse_mode="Markdown"); return
    ...
    if data.startswith("premium_analysis"):
        ...
        fi        = result.get("m01_physiognomic_index", "?")
        asym      = result.get("m30_asymmetry_pct", "?")
        asym_cls  = result.get("m30_asym_class", "")
        upper_t   = result.get("m02_upper_third_pct", "?")
        mid_t     = result.get("m03_middle_third_pct", "?")
        lower_t   = result.get("m04_lower_third_pct", "?")
        canthal_a = result.get("m08_canthal_tilt_deg", "?")
        canthal_c = result.get("m08_canthal_class", "")
        gonial_a  = result.get("m26_gonial_angle", "?")
        gonial_c  = result.get("m26_gonial_class", "")
        malar     = result.get("m33_malar_vector", "?")
        nl_angle  = result.get("m15_nasolabial_angle", "?")
        nl_status = result.get("m15_nl_status", "")
        texture   = result.get("m32_skin_texture", "?")
        def fmt(val, unit=""):
            if val in (None, "?", "н/д", "") or (isinstance(val, str) and "MediaPipe" in val):
                return None
            return f"`{val}{unit}`"

        lines = [f"🔬 *Расширенный анализ*\n\n🎯 Тир: *{result.get('tier','?')}* ({result.get('score','?')}/10)\n"]
        # Пропорции
        prop_lines = []
        if fmt(fi):          prop_lines.append(f"• Физиогн. индекс: {fmt(fi)}")
        if fmt(result.get('_fwhr','')): prop_lines.append(f"• fWHR: {fmt(result.get('_fwhr',''))}")
        thirds = [fmt(upper_t), fmt(mid_t), fmt(lower_t)]
        if all(thirds):      prop_lines.append(f"• Трети: {thirds[0]} / {thirds[1]} / {thirds[2]}")
        if prop_lines:       lines.append("📐 *Пропорции лица:*\n" + "\n".join(prop_lines) + "\n")
        # Глаза
        eye_lines = []
        if fmt(canthal_a) and canthal_c and "MediaPipe" not in str(canthal_c):
            eye_lines.append(f"• Canthal Tilt: {fmt(canthal_a, '°')} — {canthal_c}")
        if fmt(result.get('_eye_spacing','')):
            eye_lines.append(f"• Межглазное: {fmt(result.get('_eye_spacing',''))}")
        if eye_lines:        lines.append("👁 *Глаза:*\n" + "\n".join(eye_lines) + "\n")
        # Челюсть
        jaw_lines = []
        if fmt(gonial_a) and "MediaPipe" not in str(gonial_c):
            jaw_lines.append(f"• Гониальный угол: {fmt(gonial_a, '°')} — {gonial_c}")
        if jaw_lines:        lines.append("🦷 *Челюсть:*\n" + "\n".join(jaw_lines) + "\n")
        # Симметрия
        if fmt(asym):        lines.append(f"📊 *Симметрия:* {fmt(asym, '%')} — {asym_cls}")
        # Скулы
        if fmt(malar) and "MediaPipe" not in str(malar): lines.append(f"🧲 *Скулы:* {malar}")
        # Носогубный
        if fmt(nl_angle) and "MediaPipe" not in str(nl_status):
            lines.append(f"👃 *Носогубный угол:* {fmt(nl_angle, '°')} — {nl_status}")
        # Текстура
        if fmt(texture) and "MediaPipe" not in str(texture): lines.append(f"🪞 *Текстура кожи:* {texture}")
        lines.append(f"\n⚡ Осталось: {'∞' if prem else left}")
        text = "\n".join(lines)
        await query.edit_message_text(text, parse_mode="Markdown", reply_markup=menu()); return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Dating FSM — обрабатываем ПЕРВЫМ чтобы перехватить до остальных режимов
    if mode in ("dating_fsm", "dating_card_fsm"):
        if await handle_dating_text(user_id, username, text, update, context):
            return
    ...
    if mode == "awaiting_give_score":
        try:
            parts  = text.split()
            uid    = int(parts[0])
            gender = parts[1].lower() if len(parts) > 1 else "male"
            score  = float(parts[2]) if len(parts) > 2 else 50.0
            if gender not in ("male", "female"):
                await update.message.reply_text("❌ Пол должен быть male или female"); return
            score = max(0.0, min(100.0, score))
            cat_tuple = get_category(score, gender)
            cat_name  = cat_tuple[0] if isinstance(cat_tuple, tuple) else cat_tuple
            # Получаем имя пользователя из Telegram
            try:
                chat = await context.bot.get_chat(uid)
                display_name = chat.first_name or chat.username or str(uid)
            except:
                display_name = str(uid)
            save_player(uid, display_name, score, cat_name, gender)
            user_data_store.pop(user_id, None)
            await update.message.reply_text(
                f"✅ Рейтинг выдан!\n👤 {display_name} (`{uid}`)\n⭐ Счёт: {score}\n🏷 Категория: {cat_name}\n⚤ Пол: {gender}",
                parse_mode="Markdown", reply_markup=menu()
            )
            try: await context.bot.send_message(chat_id=uid, text=f"🎯 Тебе выдан рейтинг: *{score}* ({cat_name})", parse_mode="Markdown")
            except: pass
        except Exception as e:
            await update.message.reply_text(f"❌ Ошибка: {e}\n\nФормат: `ID пол счёт`\nПример: `1495553178 male 85.5`", parse_mode="Markdown")
        return
...
def get_base_analysis(image_bytes: bytes, gender: str) -> dict:
    ...
    try:
        ...
        asym = round(100 - symmetry, 1)
        if asym < 3:   asym_cls = "Высокая симметрия ✅"
        elif asym < 6: asym_cls = "Умеренная асимметрия ➡️"
        else:          asym_cls = "Выраженная асимметрия ❌"
        fi = round(fh / max(fw, 1), 3)
        return {
            "type": "base", "tier": tier, "score": score, "error": None,
            "m01_physiognomic_index": fi,
            "m02_upper_third_pct": "н/д", "m03_middle_third_pct": "н/д", "m04_lower_third_pct": "н/д",
            "m08_canthal_tilt_deg": "н/д", "m08_canthal_class": "нет данных (нужен MediaPipe)",
            "m26_gonial_angle": "н/д", "m26_gonial_class": "нет данных (нужен MediaPipe)",
            "m30_asymmetry_pct": asym, "m30_asym_class": asym_cls,
            "m33_malar_vector": "нет данных (нужен MediaPipe)",
            "m15_nasolabial_angle": "н/д", "m15_nl_status": "нет данных (нужен MediaPipe)",
            "m32_skin_texture": "нет данных (нужен MediaPipe)",
        }
    except Exception as e:
        return {"error": str(e)[:120]}


def get_opencv_premium_analysis(image_bytes: bytes, gender: str) -> dict:
    """Расширенный анализ через OpenCV — без MediaPipe."""
    import cv2, math
    try:
        nparr   = np.frombuffer(image_bytes, np.uint8)
        img_bgr = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
        ...
        # Физиогномический индекс (высота/ширина)
        fi = round(fh / max(fw, 1), 3)
        if 1.3 <= fi <= 1.5: fi_status = "Норма ✅"
        elif fi < 1.3:       fi_status = "Широкое лицо ❌"
        else:                fi_status = "Узкое лицо ❌"
        ...
        eye_spacing_score = "н/д"
        canthal_deg = "н/д"
        canthal_cls = "нет данных"
        eye_level_diff = "н/д"
        if len(eyes) >= 2:
            eyes = sorted(eyes, key=lambda e: e[0])
            ex1, ey1, ew1, eh1 = eyes[0]; ex2, ey2, ew2, eh2 = eyes[1]
            cx1 = ex1 + ew1 // 2; cy1 = ey1 + eh1 // 2
            cx2 = ex2 + ew2 // 2; cy2 = ey2 + eh2 // 2
            inter_eye = abs(cx2 - cx1)
            avg_eye_w = (ew1 + ew2) / 2
            eye_ratio = round(inter_eye / max(avg_eye_w, 1), 2)
            eye_spacing_score = f"{eye_ratio} (норма ~2.5)"
            dy = cy2 - cy1; dx = cx2 - cx1
            angle = round(math.degrees(math.atan2(dy, dx)), 1)
            canthal_deg = angle
            if angle < -2:   canthal_cls = "Positive ✅ (Hunter eyes)"
            elif angle > 2:  canthal_cls = "Negative ❌"
            else:            canthal_cls = "Neutral ➡️"
            eye_level_diff = f"{abs(cy2 - cy1)} px"

        # Текстура кожи (резкость)
        lap_var = cv2.Laplacian(img_gray[fy:fy+fh, fx:fx+fw], cv2.CV_64F).var()
        if lap_var > 200:   texture = "Высокий микрорельеф ✅"
        elif lap_var > 80:  texture = "Средний микрорельеф ➡️"
        else:               texture = "Сглажено/обработано ❌"

        # Итоговый тир
        score = 5.0
        if 1.3 <= fi <= 1.5:   score += 0.5
        if fwhr >= 1.7:         score += 1.0
        elif fwhr >= 1.5:       score += 0.5
        if symmetry >= 80:      score += 1.0
        elif symmetry >= 65:    score += 0.5
        elif symmetry < 50:     score -= 0.5
        if isinstance(canthal_deg, float) and canthal_deg < -2: score += 0.5
        ...
        return {
            "type": "opencv", "tier": tier, "score": score, "error": None,
            "m01_physiognomic_index":  f"{fi} — {fi_status}",
            "m02_upper_third_pct":     "н/д", "m03_middle_third_pct": "н/д", "m04_lower_third_pct": "н/д",
            "m08_canthal_tilt_deg":    canthal_deg,
            "m08_canthal_class":       canthal_cls,
            "m26_gonial_angle":        "н/д",
            "m26_gonial_class":        "н/д (нужен MediaPipe)",
            "m30_asymmetry_pct":       asym,
            "m30_asym_class":          asym_cls,
            "m33_malar_vector":        "н/д (нужен MediaPipe)",
            "m15_nasolabial_angle":    "н/д",
            "m15_nl_status":           "н/д (нужен MediaPipe)",
            "m32_skin_texture":        texture,
            "_eye_spacing":            eye_spacing_score,
            "_symmetry_pct":           symmetry,
            "_fwhr":                   fwhr,
        }
    except Exception as e:
        logger.error(f"OpenCV premium error: {e}", exc_info=True)
        return get_base_analysis(image_bytes, gender)


def get_premium_analysis(image_bytes: bytes, gender: str) -> dict:
    """
    Расширенный анализ — MediaPipe Tasks API (478 точек, 3D).
    33 метрики лица. Без OpenGL, работает на Render.
    """
    ...
    if not os.path.exists(MODEL_PATH):
        try:
            logger.info("Скачиваю модель MediaPipe FaceLandmarker...")
            urllib.request.urlretrieve(MODEL_URL, MODEL_PATH)
            logger.info(f"Модель скачана: {os.path.getsize(MODEL_PATH)} байт")
        except Exception as e:
            logger.error(f"Не удалось скачать модель: {e}")
            return get_base_analysis(image_bytes, gender)  # fallback на базовый
    ...
    try:
        ...
        # Запускаем MediaPipe FaceLandmarker
        ...
        mp_image   = mp_python.Image(image_format=mp_python.ImageFormat.SRGB, data=img_rgb)
        ...
        # Хелпер: получить 3D точку в пикселях + Z нормализованный
        def pt(idx):
            p = lm[idx]
            return np.array([p.x * w, p.y * h, p.z * w])  # Z масштабируем как X

        def dist3d(a, b): return np.linalg.norm(a - b)

        def angle3d(v1, v2):
            """Угол между двумя 3D векторами в градусах."""
            cos = np.dot(v1, v2) / (np.linalg.norm(v1) * np.linalg.norm(v2) + 1e-8)
            return math.degrees(math.acos(max(-1, min(1, cos))))

        # ── БАЗОВЫЕ ТОЧКИ ──
        # Глаза
        left_inner   = pt(133);  left_outer  = pt(33)
        right_inner  = pt(362);  right_outer = pt(263)
        left_pupil   = pt(468)   if len(lm) > 468 else (left_inner+left_outer)/2
        right_pupil  = pt(473)   if len(lm) > 473 else (right_inner+right_outer)/2
        left_upper   = pt(159);  left_lower  = pt(145)
        right_upper  = pt(386);  right_lower = pt(374)

        # Брови
        left_brow    = pt(70);   right_brow  = pt(300)

        # Нос
        nose_tip     = pt(4);    nose_base   = pt(2)
        nose_left    = pt(79);   nose_right  = pt(309)
        nasion       = pt(6);    pronasale   = pt(4)
        subnasale    = pt(2);    columella   = pt(94)

        # Рот
        mouth_left   = pt(61);   mouth_right = pt(291)
        upper_lip    = pt(13);   lower_lip   = pt(14)
        lip_top      = pt(0);    lip_bottom  = pt(17)
        philtrum_top = pt(164)

        # Лицо — каркас
        chin         = pt(152);  forehead    = pt(10)
        cheek_left   = pt(234);  cheek_right = pt(454)
        gonion_left  = pt(172);  gonion_right= pt(397)
        menton       = pt(152);  glabella    = pt(9)
        trichion     = pt(10);   supramen    = pt(32)
        pogonion     = pt(18)

        # Скулы
        malar_left   = pt(116);  malar_right = pt(345)

        # ── НОРМАЛИЗАЦИЯ: межзрачковое расстояние = 1.0 ──
        PD = dist3d(left_pupil, right_pupil)
        if PD < 1e-6: PD = 1.0
        def n(val): return round(val / PD, 4)

        metrics = {}

        # ── МЕТРИКА 1: Physiognomic Index (Height/Width) ──
        face_h = dist3d(trichion, menton)
        face_w = dist3d(cheek_left, cheek_right)
        metrics["m01_physiognomic_index"] = round(face_h / max(face_w, 1e-6), 3)

        # ── МЕТРИКИ 2,3,4: Правило третей ──
        eye_line     = (left_pupil + right_pupil) / 2
        upper_third  = dist3d(trichion, glabella)
        middle_third = dist3d(glabella, subnasale)
        lower_third  = dist3d(subnasale, menton)
        total_h      = upper_third + middle_third + lower_third
        metrics["m02_upper_third_pct"]  = round(upper_third  / total_h * 100, 1)
        metrics["m03_middle_third_pct"] = round(middle_third / total_h * 100, 1)
        metrics["m04_lower_third_pct"]  = round(lower_third  / total_h * 100, 1)

        # ── МЕТРИКА 5: Subnasale-Stomion vs Stomion-Menton (идеал 1:2) ──
        stomion = (upper_lip + lower_lip) / 2
        sub_to_stom  = dist3d(subnasale, stomion)
        stom_to_ment = dist3d(stomion, menton)
        metrics["m05_lip_chin_ratio"] = round(sub_to_stom / max(stom_to_ment, 1e-6), 3)

        # ── МЕТРИКА 6: Intercanthal / Palpebral Index ──
        intercanthal = dist3d(left_inner, right_inner)
        palpebral_l  = dist3d(left_inner, left_outer)
        palpebral_r  = dist3d(right_inner, right_outer)
        avg_palp     = (palpebral_l + palpebral_r) / 2
        metrics["m06_intercanthal_index"] = round(intercanthal / max(avg_palp, 1e-6), 3)

        # ── МЕТРИКА 7: Bitemporal-Bizygomatic Index ──
        bitemp = dist3d(pt(54), pt(284))
        bizygom = face_w
        metrics["m07_bitemp_bizygom"] = round(bitemp / max(bizygom, 1e-6), 3)

        # ── МЕТРИКА 8: Palpebral Slant (Canthal Tilt) 3D ──
        dx = right_outer[0] - left_outer[0]
        dy = right_outer[1] - left_outer[1]
        dz = right_outer[2] - left_outer[2]
        canthal_angle = round(math.degrees(math.atan2(dy, math.sqrt(dx**2 + dz**2))), 2)
        metrics["m08_canthal_tilt_deg"] = canthal_angle
        if canthal_angle < -2:   canthal_class = "Positive ✅ (Hunter eyes)"
        elif canthal_angle > 2:  canthal_class = "Negative ❌"
        else:                    canthal_class = "Neutral ➡️"
        metrics["m08_canthal_class"] = canthal_class

        # ── МЕТРИКА 9: Palpebral Fissure Height ──
        pf_left  = dist3d(left_upper,  left_lower)
        pf_right = dist3d(right_upper, right_lower)
        metrics["m09_palpebral_height"] = n((pf_left + pf_right) / 2)

        # ── МЕТРИКА 10: Pupil-to-Brow distance ──
        pb_left  = dist3d(left_pupil,  left_brow)
        pb_right = dist3d(right_pupil, right_brow)
        metrics["m10_pupil_brow_dist"] = n((pb_left + pb_right) / 2)

        # ── МЕТРИКА 11: Brow Slant Index ──
        brow_dx = right_brow[0] - left_brow[0]
        brow_dy = right_brow[1] - left_brow[1]
        metrics["m11_brow_slant_deg"] = round(math.degrees(math.atan2(brow_dy, brow_dx)), 2)

        # ── МЕТРИКА 12: Pupillary Distance нормализованный ──
        metrics["m12_pd_normalized"] = 1.0  # базовая константа

        # ── МЕТРИКА 13: Orbital Globe Depth (Z-глубина глаз) ──
        eye_z_depth = abs(left_pupil[2] - nasion[2])
        metrics["m13_orbital_depth"] = n(eye_z_depth)

        # ── МЕТРИКА 14: Nasal Index (Width/Length) ──
        nose_w = dist3d(nose_left, nose_right)
        nose_l = dist3d(nasion, pronasale)
        metrics["m14_nasal_index"] = round(nose_w / max(nose_l, 1e-6), 3)

        # ── МЕТРИКА 15: Nasolabial Angle (3D вектор) ──
        v_col = columella - subnasale
        v_lip = upper_lip  - subnasale
        nl_angle = angle3d(v_col, v_lip)
        metrics["m15_nasolabial_angle"] = round(nl_angle, 1)
        ideal_nl = (90, 105) if gender == "male" else (100, 120)
        metrics["m15_nl_status"] = "Норма ✅" if ideal_nl[0] <= nl_angle <= ideal_nl[1] else "Отклонение ❌"

        # ── МЕТРИКА 16: Nasofrontal Angle (3D) ──
        v_glabella = nasion - glabella
        v_dorsum   = pronasale - nasion
        metrics["m16_nasofrontal_angle"] = round(angle3d(v_glabella, v_dorsum), 1)

        # ── МЕТРИКА 17: Nasofacial Angle ──
        v_face  = menton - nasion
        v_nose  = pronasale - nasion
        metrics["m17_nasofacial_angle"] = round(angle3d(v_face, v_nose), 1)

        # ── МЕТРИКА 18: Nasal Tip Rotation ──
        v_tip = pronasale - subnasale
        horiz = np.array([1.0, 0.0, 0.0])
        metrics["m18_nasal_tip_rotation"] = round(angle3d(v_tip, horiz), 1)

        # ── МЕТРИКА 19: Dorsal Hump ──
        nose_mid = (nasion + pronasale) / 2
        nose_bridge = pt(6)
        hump = dist3d(nose_bridge, nose_mid)
        metrics["m19_dorsal_hump"] = n(hump)

        # ── МЕТРИКА 20: Mouth-to-Nose Width Index ──
        mouth_w = dist3d(mouth_left, mouth_right)
        metrics["m20_mouth_nose_index"] = round(mouth_w / max(nose_w, 1e-6), 3)

        # ── МЕТРИКА 21: Lip Volume Ratio (Upper/Lower) ──
        upper_vol = dist3d(lip_top,   upper_lip)
        lower_vol = dist3d(lower_lip, lip_bottom)
        metrics["m21_lip_ratio"] = round(upper_vol / max(lower_vol, 1e-6), 3)

        # ── МЕТРИКА 22: Oral Commissure Tilt ──
        comm_dx = mouth_right[0] - mouth_left[0]
        comm_dy = mouth_right[1] - mouth_left[1]
        metrics["m22_commissure_tilt"] = round(math.degrees(math.atan2(comm_dy, comm_dx)), 2)

        # ── МЕТРИКА 23: Cupid's Bow (philtrum width) ──
        cbow_left  = pt(37); cbow_right = pt(267)
        metrics["m23_cupids_bow"] = n(dist3d(cbow_left, cbow_right))

        # ── МЕТРИКА 24: Philtrum Height ──
        metrics["m24_philtrum_height"] = n(dist3d(philtrum_top, lip_top))

        # ── МЕТРИКА 25: Ricketts E-Line ──
        e_line_vec = pogonion - pronasale
        upper_lip_proj = upper_lip - pronasale
        e_dist = np.cross(e_line_vec, upper_lip_proj)
        metrics["m25_ricketts_eline"] = round(float(np.linalg.norm(e_dist)) / max(np.linalg.norm(e_line_vec), 1e-6), 3)

        # ── МЕТРИКА 26: Gonial Angle (3D) ──
        v_ramus  = gonion_left  - pt(234)
        v_corpus = menton       - gonion_left
        gonial   = angle3d(v_ramus, v_corpus)
        metrics["m26_gonial_angle"] = round(gonial, 1)
        if gonial < 115:   gonial_class = "Low Gonial ✅ Chad-черта"
        elif gonial < 125: gonial_class = "Average ➡️"
        else:              gonial_class = "High Gonial ❌ рецессивная"
        metrics["m26_gonial_class"] = gonial_class

        # ── МЕТРИКА 27: Pogonion Projection ──
        pog_proj = pogonion[2] - menton[2]
        metrics["m27_pogonion_proj"] = round(pog_proj / PD, 4)

        # ── МЕТРИКА 28: Mandibular Body Height ──
        metrics["m28_mandibular_height"] = n(dist3d(gonion_left, menton))

        # ── МЕТРИКА 29: Bigonial Width ──
        metrics["m29_bigonial_width"] = n(dist3d(gonion_left, gonion_right))

        # ── МЕТРИКА 30: Lateral Asymmetry Index (3D) ──
        left_side  = dist3d(left_pupil,  mouth_left)
        right_side = dist3d(right_pupil, mouth_right)
        asym = abs(left_side - right_side) / max(left_side + right_side, 1e-6) * 100
        metrics["m30_asymmetry_pct"] = round(asym, 2)
        if asym < 3:   asym_class = "Высокая симметрия ✅"
        elif asym < 6: asym_class = "Умеренная асимметрия ➡️"
        else:          asym_class = "Выраженная асимметрия ❌"
        metrics["m30_asym_class"] = asym_class

        # ── МЕТРИКА 31: Jawline Definition ──
        jaw_l = dist3d(gonion_left,  menton)
        jaw_r = dist3d(gonion_right, menton)
        metrics["m31_jawline_def"] = n((jaw_l + jaw_r) / 2)

        # ── МЕТРИКА 32: Skin Texture (Laplacian variance) ──
        face_crop = img_gray[int(forehead[1]):int(chin[1]), int(cheek_left[0]):int(cheek_right[0])]
        if face_crop.size > 0:
            import cv2 as cv2_lap
            lap_var = cv2_lap.Laplacian(face_crop, cv2_lap.CV_64F).var()
            if lap_var > 200:   texture = "Высокий микрорельеф ✅"
            elif lap_var > 80:  texture = "Средний микрорельеф ➡️"
            else:               texture = "Низкий микрорельеф (сглажено) ❌"
        else:
            texture = "Оценка ограничена"
        metrics["m32_skin_texture"] = texture

        # ── МЕТРИКА 33: Malar Vector ──
        malar_proj = (malar_left[2] + malar_right[2]) / 2 - cheek_left[2]
        if malar_proj < -0.02:  malar_class = "Positive Malar ✅"
        elif malar_proj < 0.02: malar_class = "Neutral Malar ➡️"
        else:                   malar_class = "Negative Malar ❌"
        metrics["m33_malar_vector"] = malar_class

        # ── ИТОГОВЫЙ ТИР ──
        score = 5.0
        fi = metrics["m01_physiognomic_index"]
        if 1.3 <= fi <= 1.5: score += 0.5
        if 30 <= metrics["m02_upper_third_pct"] <= 37: score += 0.3
        if 30 <= metrics["m03_middle_third_pct"] <= 37: score += 0.3
        if 30 <= metrics["m04_lower_third_pct"] <= 37: score += 0.3
        if "Positive" in canthal_class: score += 1.0
        elif "Negative" in canthal_class: score -= 0.5
        if "Норма" in metrics["m15_nl_status"]: score += 0.5
        if "Low" in gonial_class: score += 0.5
        elif "High" in gonial_class: score -= 0.5
        if metrics["m30_asymmetry_pct"] < 3: score += 0.5
        elif metrics["m30_asymmetry_pct"] > 6: score -= 0.5
        if "Positive Malar" in malar_class: score += 0.3
        score = round(max(1.0, min(10.0, score)), 1)
        if score >= 8.0:   tier = "Chad 👑"
        elif score >= 7.0: tier = "Chadlite 🔥"
        elif score >= 6.0: tier = "HTN 😎"
        ...
        metrics["score"] = score
        metrics["tier"]  = tier
        metrics["type"]  = "premium"
        metrics["error"] = None
        return metrics

    except ImportError:
        logger.warning("MediaPipe недоступен, fallback на OpenCV анализ")
        return get_opencv_premium_analysis(image_bytes, gender)
    except Exception as e:
