...
def init_db():
    ...
    """)
    for col in ["in_leaderboard INTEGER DEFAULT 1", "profile_file_id TEXT DEFAULT NULL"]:
        try:
            c.execute(f"ALTER TABLE players ADD COLUMN {col}")
        except:
            pass
    try:
        c.execute("ALTER TABLE history ADD COLUMN photo_hash TEXT")
    except:
        pass
    conn.commit()
    conn.close()
...
def save_history(user_id, score, category, photo_hash):
    conn = sqlite3.connect("ratings.db")
    c = conn.cursor()
    c.execute("INSERT INTO history (user_id, score, category, photo_hash) VALUES (?,?,?,?)",
...
def is_duplicate_photo(user_id, photo_hash):
    conn = sqlite3.connect("ratings.db")
    c = conn.cursor()
    c.execute("SELECT id FROM history WHERE user_id=? AND photo_hash=?", (user_id, photo_hash))
    row = c.fetchone()
    conn.close()
...
def set_profile_photo(user_id, file_id):
    conn = sqlite3.connect("ratings.db")
    c = conn.cursor()
    c.execute("UPDATE players SET profile_file_id=? WHERE user_id=?", (file_id, user_id))
    conn.commit()
    conn.close()
...
def get_player(user_id):
    conn = sqlite3.connect("ratings.db")
    c = conn.cursor()
    c.execute("""SELECT username,score,category,wins,losses,matches,gender,in_leaderboard,profile_file_id
...
def get_all_profiles(offset=0):
    conn = sqlite3.connect("ratings.db")
    c = conn.cursor()
    c.execute("""SELECT user_id,score,category,gender,wins,losses,matches,profile_file_id
                 FROM players ORDER BY score DESC LIMIT 1 OFFSET ?""", (offset,))
    row = c.fetchone()
...
def get_recent_photo_hashes(user_id, limit=5):
    conn = sqlite3.connect("ratings.db")
    c = conn.cursor()
    c.execute("SELECT photo_hash FROM history WHERE user_id=? ORDER BY created_at DESC LIMIT ?", (user_id, limit))
    rows = c.fetchall()
    conn.close()
...
TIPS = {
    "male": {
        "sub 3": [("Масса тела","Нормализовать массу тела.","🔴"),("Кожа","Решить проблемы с кожей у дерматолога.","🔴"),("Осанка","Исправить осанку.","🔴"),("Волосы","Подобрать подходящую стрижку.","🟡"),("Брови","Следить за бровями.","🟡"),("Здоровье","Нормализовать сон, питание, физическую активность.","🔴"),],
        "sub 5": [("Масса тела","Снизить процент жира.","🔴"),("Физ. форма","Набрать мышечную массу.","🔴"),("Кожа","Регулярный уход за кожей.","🟡"),("Волосы","Экспериментировать со стрижкой.","🟡"),("Борода","Использовать бороду для коррекции лица.","🟢"),("Зубы","Отбеливание зубов при необходимости.","🟢"),],
        "ltn":   [("Волосы","Улучшить причёску.","🔴"),("Физ. форма","Развивать шею и трапеции.","🟡"),("Кожа","Уход за кожей.","🟡"),("Осанка","Исправить осанку.","🟡"),("Стиль","Подобрать стиль одежды.","🟡"),("Фото","Улучшить качество фотографий.","🟢"),],
        "mtn":   [("Масса тела","Поддерживать низкий процент жира.","🟡"),("Кожа","Следить за кожей.","🟡"),("Физ. форма","Развивать спортивную форму.","🟡"),("Волосы","Экспериментировать с причёской.","🟢"),("Стиль","При желании — линзы вместо очков.","🟢"),("Харизма","Работать над улыбкой.","🟢"),],
        "htn":   [("Физ. форма","Поддерживать форму.","🟡"),("Кожа","Следить за кожей.","🟢"),("Волосы","Регулярно стричься.","🟢"),("Стиль","Подбирать одежду по фигуре.","🟢"),("Харизма","Работать над уверенностью.","🟢"),],
        "chad":  [("Физ. форма","Просто поддерживать текущую форму.","🟢"),("Кожа","Следить за здоровьем кожи.","🟢"),("Волосы","Не экспериментировать с неудачными стрижками.","🟢"),],
        "true adam": [("Здоровье","Поддерживать здоровье.","🟢"),("Физ. форма","Сохранять физическую форму.","🟢"),("Стиль","Не терять индивидуальный стиль.","🟢"),],
    },
    "female": {
        "sub 3": [("Кожа","Консультация дерматолога при проблемной коже.","🔴"),("Волосы","Подобрать причёску под форму лица.","🔴"),("Уход","Освоить базовый уход за кожей.","🔴"),("Зубы","При необходимости — консультация ортодонта.","🟡"),("Стиль","Подобрать подходящую оправу очков или линзы.","🟡"),],
        "sub 5": [("Волосы","Улучшить уход за волосами.","🔴"),("Кожа","Следить за состоянием кожи.","🔴"),("Макияж","Лёгкий естественный макияж.","🟡"),("Брови","Подобрать форму бровей.","🟡"),("Осанка","Работать над осанкой.","🟡"),],
        "ltb":   [("Волосы","Найти подходящую стрижку.","🔴"),("Волосы","Следить за качеством волос.","🟡"),("Кожа","Использовать уходовую косметику.","🟡"),("Стиль","Подобрать стиль одежды.","🟡"),],
        "mtb":   [("Кожа","Регулярный уход за кожей.","🟡"),("Макияж","Аккуратный макияж по желанию.","🟢"),("Физ. форма","Поддерживать физическую форму.","🟡"),("Стиль","Экспериментировать с образом.","🟢"),],
        "htb":   [("Уход","Поддерживать текущий уход.","🟢"),("Волосы","Следить за здоровьем волос.","🟢"),("Здоровье","Хороший сон.","🟢"),("Кожа","Защита кожи от солнца.","🟢"),("Фото","Подбирать качественные фотографии.","🟢"),],
        "stacy": [("Физ. форма","Поддерживать форму.","🟢"),("Кожа","Беречь кожу.","🟢"),("Волосы","Регулярный уход за волосами.","🟢"),("Косметика","Не злоупотреблять косметическими процедурами.","🟢"),],
        "true eve": [("Здоровье","Поддерживать здоровье.","🟢"),("Кожа","Регулярный уход за кожей и волосами.","🟢"),("Стиль","Сохранять естественный внешний вид.","🟢"),("Физ. форма","Следить за физической формой.","🟢"),],
    }
}
...
def check_edit_level(img_gray) -> float:
    """Оценивает уровень обработки фото (0-100, чем выше — тем больше фильтров)."""
    import cv2
    # Лапласиан — обработанные фото имеют меньше шума
    lap_var = cv2.Laplacian(img_gray, cv2.CV_64F).var()
    # Нормализуем: низкий шум = высокая обработка
    edit_score = max(0, min(100, 100 - (lap_var / 5)))
    return round(edit_score, 1)

def check_screen_photo(img_bgr) -> bool:
    """Проверяет признаки фото с экрана (муаровые паттерны, пиксели сетки)."""
    import cv2
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    # FFT анализ — фото с экрана имеют регулярные паттерны
    f = np.fft.fft2(gray)
    ...
    outer_region[h//2-20:h//2+20, w//2-20:w//2+20] = 0
    # Если много энергии в высоких частотах — вероятно экран
    ratio = outer_region.mean() / max(center_region.mean(), 1)
    return ratio > 0.15
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "set_profile_photo":
        ...
        await update.message.reply_text("✅ Фото профиля обновлено!", reply_markup=main_menu_keyboard())
        return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "profile":
        ...
        if not row:
            ...
            return
        uname, score, cat, wins, losses, matches, gender, in_lb, profile_pic = row
    ...
    if data == "set_pfp":
        ...
        await query.edit_message_text("📸 Пришли фото которое хочешь поставить на профиль!")
        return

    if data == "toggle_lb":
        new_val = toggle_leaderboard(user_id)
        status  = "✅ Ты теперь в таблице лидеров!" if new_val else "❌ Ты скрыт из таблицы лидеров."
    ...
    if data.startswith("browse_"):
        ...
        row, total = get_all_profiles(offset)
        if not row or total == 0:
            await query.edit_message_text("😔 Профилей пока нет.", reply_markup=main_menu_keyboard())
            return
        uid, score, cat, gender, wins, losses, matches, profile_file_id = row
        icon    = "👨" if gender == "male" else "👩"
        winrate = round(wins/matches*100) if matches > 0 else 0
        text    = (
            f"{icon} *Профиль #{offset+1} из {total}*\n\n"
            f"🎯 Категория: *{cat.upper()}*\n"
            f"📊 Балл: *{score:.0f}/100*\n"
            f"⚔️ Матчей: {matches} | {wins}W/{losses}L\n"
            f"📈 Винрейт: {winrate}%"
        ...
        nav = []
        if offset > 0:
            nav.append(InlineKeyboardButton("⬅️", callback_data=f"browse_{offset-1}"))
        if offset < total - 1:
            nav.append(InlineKeyboardButton("➡️", callback_data=f"browse_{offset+1}"))
        ...
        try:
            ...
            if is_duplicate_photo(user_id, photo_hash):
                await query.edit_message_text(
                    "⚠️ Ты уже отправлял это фото раньше!\n\nДля честного результата пришли новое фото.",
                    reply_markup=main_menu_keyboard()
                )
                return
            ...
            # Предупреждение о фото с экрана
            screen_warn = "\n⚠️ *Похоже на фото с экрана!* Результат может быть неточным.\n" if is_screen else ""

            # Уровень обработки
            if edit_level < 30:
                edit_text = "🟢 Минимальная"
            elif edit_level < 60:
                edit_text = "🟡 Умеренная"
            else:
                edit_text = "🔴 Сильная"
            ...
            if mode == "rate":
                ...
                text = (
                    ...
                    f"🖼 Обработка фото: {edit_text} ({edit_level:.0f}%)\n"
                )
                keyboard = InlineKeyboardMarkup([
