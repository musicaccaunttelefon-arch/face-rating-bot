...
# Устанавливаем системную зависимость OpenCV если нет
import subprocess, sys
try:
    import cv2
except Exception:
    subprocess.run(["apt-get", "install", "-y", "libgl1-mesa-glx", "libglib2.0-0"], capture_output=True)
    subprocess.run([sys.executable, "-m", "pip", "install", "--upgrade", "opencv-python-headless"], capture_output=True)
...
def can_access(user_level, feature_level):
    return LEVEL_ORDER.get(user_level, 0) >= LEVEL_ORDER.get(feature_level, 0)
...
def analyze_face(image_bytes, gender):
    try:
        ...
        # MediaPipe FaceLandmarker — 478 точек лица
        ...
        cheek_l  = pt(234);   cheek_r   = pt(454)

        scores     = {}
        face_width = np.linalg.norm(cheek_r - cheek_l)
        ...
        t1 = eye_line - forehead[1]
        t2 = nose_base[1] - eye_line
        t3 = chin[1] - nose_base[1]
        total_h = t1+t2+t3
        if total_h > 0:
            ideal = 1/3
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "updates":
        ...
        elif is_admin(username, user_id): level = "admin"
        else: level = "public"
