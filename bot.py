...
def init_dating_db():
    conn = new_conn(); c = conn.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS dating_profiles (
        user_id   BIGINT PRIMARY KEY, name TEXT, age INTEGER,
        city TEXT, bio TEXT, photo_fid TEXT, is_active BOOLEAN DEFAULT TRUE,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
    c.execute("""CREATE TABLE IF NOT EXISTS dating_reactions (
        id SERIAL PRIMARY KEY, from_id BIGINT, to_id BIGINT,
        reaction TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, UNIQUE(from_id,to_id))""")
    c.execute("""CREATE TABLE IF NOT EXISTS dating_matches (
    ...
    for col in ["is_dating BOOLEAN DEFAULT FALSE",
