[21.09.2026 1:36] нереальный талант: import reimport loggingimport osimport numpy as npimport asyncioimport hashlibimport mathimport urllib.requestfrom datetime import datetime, timedeltafrom aiohttp import webfrom urllib.parse import urlparse, unquote
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkupfrom telegram.ext import (
    Application, CommandHandler, MessageHandler,
    CallbackQueryHandler, ContextTypes, filters,
)import pg8000

logging.basicConfig(level=logging.INFO)logger = logging.getLogger(name)
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "YOUR_BOT_TOKEN_HERE")RENDER_URL     = os.environ.get("RENDER_URL", "")DATABASE_URL   = os.environ.get("DATABASE_URL", "")PORT           = int(os.environ.get("PORT", 8080))ADMIN_USERNAME = "nerealnytalanty"ADMIN_CARD     = "2202208039479622\nZELENTSOV IVAN"
user_data_store    = {}matchmaking_queues = {"male": [], "female": [], "premium_male": [], "premium_female": []}match_store        = {}
# ──────────────────────────────────────────────# БЕЗОПАСНЫЙ БЛОК РАБОТЫ С БД# ──────────────────────────────────────────────
def get_conn():
    """Надежный парсинг DATABASE_URL для пулеров Supabase и драйвера pg8000"""
    try:
        # Регулярное выражение корректно вытаскивает юзера с точкой и пароль со спецсимволами
        pattern = r"str?://([^:]+):([^@]+)@([^:]+):(\d+)/([^?]+)"
        ...
        if match:
            ...
            # Резервный вариант на случай нестандартной строки
            p = urlparse(DATABASE_URL)
            db_user = unquote(p.username) if p.username else None
            db_pass = unquote(p.password) if p.password else None
            db_host, db_port, db_name = p.hostname, (p.port or 5432), p.path.lstrip("/")
        ...
        raise e
class SafeCursor:
    """Адаптер курсора, переводящий %s в нативные плейсхолдеры pg8000 ($1, $2...)"""
    def init(self, cursor):
        self.cursor = cursor

    def execute(self, sql, params=None):
        if params:
            # Считаем, сколько $ уже захардкожено в запросе, чтобы не сбить нумерацию
            existing_dollars = len(re.findall(r'\$\d+', sql))
            counter = [existing_dollars]
            ...
            # Меняем только %s
            processed_sql = re.sub(r"%s", replace_placeholder, sql)
            self.cursor.execute(processed_sql, list(params))
        else:
...
class SafeConnection:
    def init(self, conn):
        self.conn = conn
    def cursor(self): return SafeCursor(self.conn.cursor())
    def commit(self): self.conn.commit()
    def close(self): self.conn.close()
def new_conn():
    return SafeConnection(get_conn())
def init_db():
    conn = new_conn()
    c = conn.cursor()
    c.execute("""
...
[21.09.2026 1:36] нереальный талант: val = c.fetchone()
    conn.close()
...
def get_leaderboard(gender=None, premium_only=False):
    ...
    params = []
    if gender:
        base += " AND gender=$1"; params.append(gender)
    if premium_only:
...
def get_all_profiles(offset=0, premium_only=False):
    ...
    if premium_only:
        ...
        c2 = conn.conn.cursor() # Используем нативный курсор для параллельного селекта
...
def get_history(user_id):
    conn = new_conn()
    c = conn.cursor()
    c.execute("SELECT score,category,created_at FROM history WHERE user_id=$1 ORDER BY created_at DESC LIMIT 10", (user_id,))
    rows = c.fetchall()
    conn.close()
    return rows
...
def set_premium(user_id, months):
    ...
    c.execute("UPDATE players SET is_premium=TRUE, premium_until=$1 WHERE user_id=$2", (until, user_id))
    conn.commit()
    conn.close()
    return until
...
conn = new_conn()
c = conn.cursor()
c.execute("UPDATE players SET is_premium=FALSE, premium_until=NULL WHERE user_id=$1", (user_id,))
conn.commit()
conn.close()
def set_beta(user_id, val):
conn = new_conn()
c = conn.cursor()
c.execute("UPDATE players SET is_beta=$1 WHERE user_id=$2", (val, user_id))
conn.commit()
conn.close()
...
conn = new_conn()
c = conn.cursor()
c.execute("INSERT INTO premium_requests (user_id,username,plan) VALUES ($1,$2,$3)", (user_id, username, plan))
conn.commit()
conn.close()
...
conn = new_conn()
c = conn.cursor()
c.execute("SELECT user_id,plan FROM premium_requests WHERE id=$1", (req_id,))
row = c.fetchone()
if row:
user_id, plan = row
...
until = datetime.now() + timedelta(days=30*months)
c.execute("UPDATE players SET is_premium=TRUE, premium_until=$1 WHERE user_id=$2", (until, user_id))
c.execute("UPDATE premium_requests SET status='approved' WHERE id=$1", (req_id,))
conn.commit()
conn.close()
...
conn = new_conn()
c = conn.cursor()
c.execute("SELECT user_id FROM premium_requests WHERE id=$1", (req_id,))
row = c.fetchone()
c.execute("UPDATE premium_requests SET status='rejected' WHERE id=$1", (req_id,))
conn.commit()
conn.close()
...
conn = new_conn()
c = conn.cursor()
c.execute("INSERT INTO updates (title,content,level) VALUES ($1,$2,$3)", (title, content, level))
conn.commit()
conn.close()
...
[21.09.2026 1:36] нереальный талант: "ltn": [("Волосы","Улучшить причёску.","🔴"),("Физ. форма","Развивать шею и трапеции.","🟡"),("Кожа","Уход за кожей.","🟡"),("Осанка","Исправить осанку.","🟡"),("Стиль","Подобрать стиль одежды.","🟡"),],
...
scores = {}
scores["golden_ratio"] = max(0, 100 - abs(fh/max(fw,1)-1.618)/1.618180)
...
scores["symmetry"] = max(0, 100 - min(100, diff.mean()/128200))
...
scores["eye_spacing"] = max(0, 100-abs(abs((ex2+ew2//2)-(ex1+ew1//2))/max((ew1+ew2)/2,1)-2.5)/2.5100)
scores["eye_level"] = max(0, 100-abs(ey1-ey2)/max(fh,1)500)
else:
scores["eye_spacing"] = 50; scores["eye_level"] = 50
scores["face_clarity"] = min(100, fwfh/max(w*h,1)*400)
...
## ──────────────────────────────────────────────## МЕНЮ## ──────────────────────────────────────────────
PREMIUM_TEXT = """💎 Premium «Здесь могли бы быть мы»
...
• ❤️ Поддержка проекта — средства идут на развитие бота.
━━━━━━━━━━━━━━━
💳 Тарифы:
• 1 месяц — 99 ₽
• 3 месяца — 249 ₽
...
[21.09.2026 1:36] нереальный талант: if await handle_dating_photo(user_id, photo.file_id, update, context): return
...
nav = []
if offset > 0: nav.append(InlineKeyboardButton("⬅️", callback_data=f"{prefix}{offset-1}"))
if offset < total-1: nav.append(InlineKeyboardButton("➡️", callback_data=f"{prefix}{offset+1}"))
...
[21.09.2026 1:36] нереальный талант: caption=text, parse_mode="Markdown", reply_markup=keyboard)
return
except: pass
await query.edit_message_text(text + "\n\n_Фото профиля не установлено", parse_mode="Markdown", reply_markup=keyboard); return
...
_btn = f"🔬 Расширенный анализ ({'∞' if _prem else left} ⚡)"
keyboard = InlineKeyboardMarkup([
[InlineKeyboardButton("💡 Советы по улучшению", callback_data=f"tips|{category}|{gender}")],
...
[21.09.2026 1:36] нереальный талант: if data == "give_beta":
if not is_admin(username): return
...
ref_link = f"t.me{bot_info.username}?start=ref_{user_id}"
try:
...
if premium:
search_queues.append(mm_gender)
...
match_store[opponent_
id] = {"opponent_id": user_id, "my_score": None, "opp_score": None, "my_cat": None, "opp_cat": None, "my_emoji": None, "opp_emoji": None, "opp_name": user.first_name or "Аноним"}
...
[21.09.2026 1:36] нереальный талант: if user_id not in match_store:
...
[21.09.2026 1:36] нереальный талант: right_inner = pt(362); right_outer = pt(263)
...
canthal_angle = round(math.degrees(math.atan2(dy, math.sqrt(dx2 + dz2))), 2)
...
logger.warning("MediaPipe недоступен, fallback на базовый анализ")
return get_base_analysis(image_bytes, gender)
except Exception as e:
logger.error(f"Premium analysis error: {e}", exc_info=True)
return get_base_analysis(image_bytes, gender)
[21.09.2026 1:36] нереальный талант: ## ══════════════════════════════════════════════════════════════════## МОДУЛЬ 2: РЕФЕРАЛЬНАЯ СИСТЕМА## ══════════════════════════════════════════════════════════════════
...
## ══════════════════════════════════════════════════════════════════## МОДУЛЬ 3: DATING СИСТЕМА (ИСПРАВЛЕНА SQL-ОШИБКА)## ══════════════════════════════════════════════════════════════════
def init_dating_db():
tables = [
...
logger.warning(f"Dating table setup error: {e}")
...
"""Ищет следующий подходящий профиль, исключая просмотренные через чистый SQL подзапрос"""
try:
conn = new_conn(); c = conn.cursor()
...
my_age, my_city = me
# Базовый SQL-фильтр для исключения уже обработанных анкет и самого себя
exclude_subquery = "AND dp.user_id != $1 AND dp.user_id NOT IN (SELECT to_id FROM dating_reactions WHERE from_id = $1)"
queries = [
(f"SELECT dp.user_id FROM dating_profiles dp JOIN players p ON p.user_id=dp.user_id WHERE dp.is_active=TRUE AND p.is_dating=TRUE {exclude_subquery} AND LOWER(dp.city)=LOWER($2) AND ABS(dp.age-$3)<=2 ORDER BY RANDOM() LIMIT 1", [user_id, my_city, my_age]),
(f"SELECT dp.user_id FROM dating_profiles dp JOIN players p ON p.user_id=dp.user_id WHERE dp.is_active=TRUE AND p.is_dating=TRUE {exclude_subquery} AND LOWER(dp.city)=LOWER($2) ORDER BY RANDOM() LIMIT 1", [user_id, my_city]),
(f"SELECT dp.user_id FROM dating_profiles dp JOIN players p ON p.user_id=dp.user_id WHERE dp.is_active=TRUE AND p.is_dating=TRUE {exclude_subquery} AND ABS(dp.age-$2)<=2 ORDER BY RANDOM() LIMIT 1", [user_id, my_age]),
(f"SELECT dp.user_id FROM dating_profiles dp JOIN players p ON p.user_id=dp.user_id WHERE dp.is_active=TRUE AND p.is_dating=TRUE {exclude_subquery} ORDER BY RANDOM() LIMIT 1", [user_id]),
]
result = None
...
[21.09.2026 1:53] нереальный талант: async def dating_show_card(user_id, chat_id, context):
...
, name, age, city, bio, photo_fid, _ = profile
...
[21.09.2026 1:53] нереальный талант: return True
