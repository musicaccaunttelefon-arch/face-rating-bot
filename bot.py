...
def check_edit_level(img_gray):
    ...
    # Высокая резкость = низкая обработка
    return round(max(0, min(100, 100 - min(lap_var / 3, 100))), 1)
...
def analyze_face(image_bytes, gender):
    try:
        ...
        # Обрезаем лишнее по бокам (волосы расширяют bbox) — берём центр 80%
        pad_x = int(fw * 0.10)
        pad_y_top = int(fh * 0.12)  # срезаем лоб/волосы сверху
        face_img = img_gray[fy + pad_y_top: fy + fh, fx + pad_x: fx + fw - pad_x]
        fc_w = fw - 2 * pad_x
        fc_h = fh - pad_y_top

        scores = {}

        # 1. Симметрия — по обрезанному лицу
        mid = fc_w // 2
        ...
        raw_sym = max(0, 100 - diff.mean() / 128 * 150)  # было *200 — слишком жёстко
        scores["symmetry"] = min(100, raw_sym)

        # 2. Пропорции лица (fi вместо наивного golden_ratio)
        fi = fc_h / max(fc_w, 1)
        # Идеал для мужчин ~1.35-1.5, для женщин ~1.4-1.6
        ideal_min = 1.35 if gender == "male" else 1.40
        ideal_max = 1.50 if gender == "male" else 1.60
        if ideal_min <= fi <= ideal_max:
            scores["golden_ratio"] = 90
        elif fi < ideal_min:
            scores["golden_ratio"] = max(40, 90 - (ideal_min - fi) * 150)
        else:
            scores["golden_ratio"] = max(40, 90 - (fi - ideal_max) * 150)

        # 3. Глаза
        face_top = img_gray[fy + pad_y_top: fy + fh // 2, fx + pad_x: fx + fw - pad_x]
        eyes = eye_cascade.detectMultiScale(face_top, 1.05, 3)
        if len(eyes) >= 2:
            eyes = sorted(eyes, key=lambda e: e[0])
            ex1, ey1, ew1, eh1 = eyes[0]; ex2, ey2, ew2, eh2 = eyes[1]
            cx1 = ex1 + ew1 // 2; cx2 = ex2 + ew2 // 2
            inter_eye  = abs(cx2 - cx1)
            avg_eye_w  = (ew1 + ew2) / 2
            eye_ratio  = inter_eye / max(avg_eye_w, 1)
            # Идеал ~2.5 (расстояние между глазами = ширина одного глаза * 2.5)
            scores["eye_spacing"] = max(0, 100 - abs(eye_ratio - 2.5) / 2.5 * 80)
            scores["eye_level"]   = max(0, 100 - abs(ey1 - ey2) / max(fc_h, 1) * 400)
        else:
            scores["eye_spacing"] = 50
            scores["eye_level"]   = 50

        # 4. Чёткость — размер лица относительно фото
        scores["face_clarity"] = min(100, fc_w * fc_h / max(w * h, 1) * 500)

        # Веса
        weights = {"symmetry": 0.35, "golden_ratio": 0.20, "eye_spacing": 0.20, "eye_level": 0.10, "face_clarity": 0.15}
        total = max(0, min(100, sum(scores[k] * weights[k] for k in weights)))

        # Базовый буст для всех — реальные люди редко ниже 35
        total = max(35, total)
