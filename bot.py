...
def init_db():
    ...
    # Таблица для хранения file_id фото
    try:
        c.execute("""
        ...
        logger.warning(f"bot_photos table: {e}")
    ...
    try:
        ...
        ('lose_photo',     'Фото поражения',    '💀', 'lose_photo',       'admin', True),
        ('premium_photo',  'Фото покупки прем.','💎', 'premium_photo',    'admin', True),
        ('approve_photo',  'Фото одобрения',    '✅', 'approve_photo',    'admin', True),
        ('reject_photo',   'Фото отклонения',   '❌', 'reject_photo',     'admin', True),
        ]
    ...
    for feat in photo_features:
        try:
            c.execute("""INSERT INTO features (name,label,emoji,callback,access_level,enabled)
                         VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (name) DO NOTHING""", list(feat))
            conn.commit()
        except Exception as e:
            logger.warning(f"Photo feature migration: {e}")
...
def get_photo_file_id(name):
    """Получает file_id фото из БД."""
    try:
        conn = new_conn()
        c = conn.cursor()
        c.execute("SELECT file_id FROM bot_photos WHERE name=%s", (name,))
        row = c.fetchone()
        conn.close()
        return row[0] if row else None
    except Exception:
...
def save_photo_file_id(name, file_id):
    """Сохраняет file_id фото в БД."""
    conn = new_conn()
    c = conn.cursor()
    c.execute("INSERT INTO bot_photos (name, file_id) VALUES (%s,%s) ON CONFLICT (name) DO UPDATE SET file_id=EXCLUDED.file_id", (name, file_id))
    conn.commit()
    conn.close()
...
async def send_photo_smart(bot, chat_id, photo_name, fallback_url, caption="", reply_markup=None, parse_mode="Markdown"):
    """Отправляет фото: сначала по file_id из БД, если нет — по URL и сохраняет file_id."""
    file_id = get_photo_file_id(photo_name)
    try:
        if file_id:
            msg = await bot.send_photo(chat_id=chat_id, photo=file_id, caption=caption,
                                        reply_markup=reply_markup, parse_mode=parse_mode)
        else:
            msg = await bot.send_photo(chat_id=chat_id, photo=fallback_url, caption=caption,
                                        reply_markup=reply_markup, parse_mode=parse_mode)
            # Сохраняем file_id для следующих отправок
            if msg.photo:
                save_photo_file_id(photo_name, msg.photo[-1].file_id)
        return msg
    except Exception as e:
        logger.error(f"send_photo_smart error: {e}")
        return None
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if can_see_photo("welcome_photo", user_id, username):
        try:
            welcome_msg = await send_photo_smart(
                context.bot, update.message.chat_id,
                "welcome", "https://raw.githubusercontent.com/musicaccaunttelefon-arch/face-rating-bot/main/file_00000000f83881f48f67913ced6804f0.png",
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "awaiting_screenshot":
        ...
        if can_see_photo("premium_photo", user_id, _uname):
            try:
                await send_photo_smart(
                    context.bot, update.message.chat_id, "premium",
                    "https://raw.githubusercontent.com/musicaccaunttelefon-arch/face-rating-bot/main/file_00000000537c81f49e012d5ed0812aa6.png",
                    caption="✅ Скриншот получен! Ожидай подтверждения от владельца.",
                    reply_markup=main_menu_keyboard(user_id, _uname)
                )
            except Exception:
                await update.message.reply_text(
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("approve_"):
        ...
        if approved_user_id:
            try:
                _apr_row = get_player(approved_user_id)
                _apr_uname = _apr_row[0] if _apr_row else ""
                if can_see_photo("approve_photo", approved_user_id, _apr_uname):
                    await send_photo_smart(
                        context.bot, approved_user_id, "approve",
                        "https://raw.githubusercontent.com/musicaccaunttelefon-arch/face-rating-bot/main/file_0000000030c8820a923e35e06356e7a7.png",
                        caption=f"🎉 Твой Premium подтверждён на {months} мес.! Спасибо за поддержку! 💎"
                    )
                else:
    ...
    if data.startswith("reject_"):
        ...
        if rejected_uid:
            try:
                _rej_row = get_player(rejected_uid)
                _rej_uname = _rej_row[0] if _rej_row else ""
                if can_see_photo("reject_photo", rejected_uid, _rej_uname):
                    await send_photo_smart(
                        context.bot, rejected_uid, "reject",
                        "https://raw.githubusercontent.com/musicaccaunttelefon-arch/face-rating-bot/main/file_000000003b50820aadde98e8c03e7122.png",
                        caption="❌ Твоя заявка на Premium отклонена. Свяжись с @Xiliiiwhy для уточнения деталей."
                    )
                else:
    ...
    if opponent_id:
        ...
        if can_see_photo("mm_photo", user_id, username):
            try:
                await query.message.delete()
                await send_photo_smart(
                    context.bot, query.message.chat_id, "mm",
                    "https://raw.githubusercontent.com/musicaccaunttelefon-arch/face-rating-bot/main/file_00000000720881f4804b36373887ac43.png",
...
async def process_match_result(query, user_id, score, category, emoji, context):
    ...
    winner_username = query.from_user.username if query and query.from_user else ""
    if "ПОБЕДА" in my_text and can_see_photo("win_photo", user_id, winner_username):
        try:
            await query.message.delete()
            await send_photo_smart(
                context.bot, query.message.chat_id, "win",
                "https://raw.githubusercontent.com/musicaccaunttelefon-arch/face-rating-bot/main/file_00000000dcb8820a8fcbd341833c5c91.png",
                caption=my_text, reply_markup=main_menu_keyboard(user_id)
            )
        except Exception:
            await query.edit_message_text(my_text, parse_mode="Markdown", reply_markup=main_menu_keyboard(user_id))
    elif "ПОРАЖЕНИЕ" in my_text and can_see_photo("lose_photo", user_id, winner_username):
        try:
            await query.message.delete()
            await send_photo_smart(
                context.bot, query.message.chat_id, "lose",
                "https://raw.githubusercontent.com/musicaccaunttelefon-arch/face-rating-bot/main/sdelai-mechi-krasnym-tsvetom-kak-i-nadpi_99IMoGdNWm-Z_5zRQex2CQ_tIShYbWjTYWy_DxcKp2StA.jpg",
                caption=my_text, reply_markup=main_menu_keyboard(user_id)
            )
        except Exception:
            await query.edit_message_text(my_text, parse_mode="Markdown", reply_markup=main_menu_keyboard(user_id))
